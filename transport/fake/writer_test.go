...
func TestWrite_Fake(t *testing.T) {
	var innerWriter collectWrites
	fakeWriter := NewWriter(&innerWriter, 3)
	n, err := fakeWriter.Write([]byte("Request"))
...
func TestWrite_ShortWrite(t *testing.T) {
	var innerWriter collectWrites
	fakeWriter := NewWriter(&innerWriter, 10)
	n, err := fakeWriter.Write([]byte("Request"))
	require.NoError(t, err)
	require.Equal(t, 7, n)
...
func TestWrite_Zero(t *testing.T) {
	var innerWriter collectWrites
	fakeWriter := NewWriter(&innerWriter, 0)
	n, err := fakeWriter.Write([]byte("Request"))
	require.NoError(t, err)
	require.Equal(t, 7, n)
	require.Equal(t, [][]byte{[]byte("Request")}, innerWriter.writes)
...
func TestWrite_NeedsTwoWrites(t *testing.T) {
	var innerWriter collectWrites
	fakeWriter := NewWriter(&innerWriter, 5)
	n, err := fakeWriter.Write([]byte("Re"))
	require.NoError(t, err)
	require.Equal(t, 2, n)
	n, err = fakeWriter.Write([]byte("quest"))
	require.NoError(t, err)
	require.Equal(t, 5, n)
...
func TestWrite_Compound(t *testing.T) {
	var innerWriter collectWrites
	fakeWriter := NewWriter(NewWriter(&innerWriter, 4), 1)
	n, err := fakeWriter.Write([]byte("Request"))
	require.NoError(t, err)
	require.Equal(t, 7, n)
...
func TestReadFrom(t *testing.T) {
	fakeWriter := NewWriter(&bytes.Buffer{}, 3)
...
func TestReadFrom_ShortRead(t *testing.T) {
	fakeWriter := NewWriter(&bytes.Buffer{}, 10)
	rf, ok := fakeWriter.(io.ReaderFrom)
	require.True(t, ok)
	cr := &collectReader{Reader: bytes.NewReader([]byte("Request1"))}
	n, err := rf.ReadFrom(cr)
